			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$list['quantity']}}</td>
						@if($range->isEmpty())
							<td>{{$list['product_info']['price']}}</td>
						@endif
						@foreach($list['product_info']['ranges'] as $ranges)
							@if($ranges['from'] <= $list['quantity'] && $ranges['to'] >= $list['quantity'])
							<td>{{$ranges['price']}}</td>
							@endif
						@endforeach
					...
					<tr>
						...
						<td><b>{{
                        array_reduce($vendor_data,function($total,$item){
                            $total+=  $item['amount'];
                            return $total; 
                        },0)    
